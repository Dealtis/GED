...
console.log("(▀¯▀) GED POLE START v1.1.1 Dev(▀¯▀)");
                ...
                switch (doc[0].filename.slice(-3)) {
                    ...
                    case "pdf":
                        ...
                        exec('pdftk' + out + " cat output temp/" + rows[0].numequinoxe + ".pdf", function(error, stdout, stderr) {
                            if (error) {
                                console.error(error);
                                res.sendFile('E:/www/xamp/htdocs/GED/dev/errorGed/errorGed.html');
                            } else {
...
io.on('creaSociete', function(soc) {
...
exports.sendMail = function(mailData) {
    ...
    //Msg Error
fs.readFile('mail/Error/error.html', function(err, html) {
    ...
    exports.sendErrorMsg = function(societe, errCode, errMessage) {
        sendErrorMsg({
            to: 'rvanardois@dealtis.fr'
        }, {
            societe: societe,
...
//Msg Restart
fs.readFile('mail/Restart/restart.html', function(err, html) {
    var sendRestartMsg = transporter.templateSender({
        subject: 'GED Restart',
        html: html
    }, {
        from: 'dealtis.ged@gmail.com',
    });
    exports.sendRestartMsg = function() {
        sendRestartMsg({
            to: 'rvanardois@dealtis.fr'
        }, {
            timeRestart: new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '')
        }, function(err, info) {
            if (err) {
                console.log('Error' + err);
            } else {
                console.log('Restart Msg sent');
            }
        });
    };
});
